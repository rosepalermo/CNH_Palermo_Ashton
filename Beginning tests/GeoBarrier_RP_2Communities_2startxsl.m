...
Wpert = We; %width of beach where community is, let's say it doesn't drop below 150 somehow later
...
%perterbation = shoreline position of an unnurtured/ eroding beach
%jjcom = ceil((.5-pertpart/2)*Yn):floor((0.5+pertpart/2)*Yn);
jjcom = zeros(size(Yi));
jjcom1 = 31:50; %1 represents community 1
jjcom2 = 141:161; %2 represents community 2
%xsl(jjcom1)=xbb(jjcom1)-Wpert1;   % sl of shoreline in front of communities
%xsl(jjcom2)=xbb(jjcom2)-Wpert2;
%notpert = setdiff(Yi,jjcom); %part of the shoreline not on community
...
for i=1:ts
    ...
    %NEED TO UPDATE NOURISHMENT FOR 2 COMMUNITIES STARTING AT DIFFERENT
    %PLACES%
       ...
       %component
        ...
        if xsl(jjcom1(1):jjcom1(end)) > 820
            xsl(jjcom1) = xsl(jjcom1) -20;
            tnourished(i) = 1;
            disp('nourished 1')
        else
        ...
        if xsl(jjcom2(1):jjcom2(end)) > 820
    ...
    if (mod(i,plotnum)- 1 == 0)
        ...
        plot(Y(jjcom1)/1000,xsl(jjcom1)/1000,'k','linewidth',2)
        plot(Y(jjcom2)/1000,xsl(jjcom2)/1000,'k','linewidth',4)
        
  %      patch([Y/1000 flip(Y/1000)],[xsl/1000 flip(xbb/1000)],'k')
        
        xlabel('alongshore location (km)')
        ylabel('onshore location (km)')
        legend('Shoreline A','Backbarrier A','Community 1','Community 2','location','se')
...
title('Adjacent communities and width start = We')
