...
if plot_on
    ...
    plot(Y/1000,xsl_cr,'b', 'linewidth',2)
        axis([Y/1000 Y(Yn)/1000 min(xsl_cr) max(xsl_cr)+1])
...
QowB_saveall = QowB_saveall(:,(1:length(Y)));
QowH_saveall = QowH_saveall(:,(1:length(Y)));
Qow_saveall = Qow_saveall(:,(1:length(Y)));
Qsf_saveall = Qsf_saveall(:,(1:length(Y)));
Qast_saveall = Qast_saveall(:,(1:length(Y)));
W_saveall = W_saveall(:,(1:length(Y)));
H_saveall = H_saveall(:,(1:length(Y)));
xsl_saveall = xsl_saveall(:,(1:length(Y)));
if save_on
    if xsonly
...
%         filename = sprintf('NAT_%s_OW%d_K%d_SLa%d_diff%d',shape,Qow_max,Ksf,sl_a*1000,astfac*100);
    ...
    if plot_on
        if community_on
            ...
            saveas(h,figname)
            save(savefilename,'QowB_save','QowH_save','Qow_save','Qsf_save','Qast_save','H_save','W_save','t','Y','sl_a','sl_b','Qow_max','Ksf','shape','jplot','community_on','save_on','astfac','xbb_save','xsl_saveall','filename','foldername','com','llxldot_saveall')
        else
            fig = '.png'; figname = strcat(foldername,filename,fig);
            saveas(h,figname)
